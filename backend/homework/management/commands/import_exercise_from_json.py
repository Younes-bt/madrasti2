...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for idx, item in enumerate(items, start=1):
            ...
            if not isinstance(item, dict):
                self.stdout.write(self.style.ERROR(f'Item {idx}: Expected object, got {type(item).__name__}'))
                continue
            if 'lesson_info' not in item:
            ...
            # Find the lesson
            lesson = self._find_lesson(item['lesson_info'])
            if not lesson:
                continue
            ...
            with transaction.atomic():
                ...
                # Create questions
                questions_created = self._create_questions(exercise, item['questions'])
                total_created += 1
        ...
        if len(items) > 1:
            self.stdout.write(self.style.SUCCESS(f'\nDone. Imported {total_created}/{len(items)} exercises.'))
        ...
        if 'lesson_id' in lesson_info and lesson_info['lesson_id']:
            try:
                ...
                self.stdout.write(self.style.WARNING(
                    f'Lesson ID {lesson_info["lesson_id"]} not found. Trying other methods...'
                ))
    ...
    def _create_exercise(self, lesson, exercise_data, admin_user, delete_existing):
        ...
        if delete_existing:
            ...
            if count > 0:
                existing.delete()
                self.stdout.write(self.style.WARNING(f'Deleted {count} existing exercise(s)'))
    ...
    def _create_questions(self, exercise, questions_data):
        ...
        for q_data in questions_data:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(
                    f'Error creating question {q_data.get("order", "?")}: {str(e)}'
                ))
                continue
