...
# 📢 Backend & Content Update: The "Gamification" Upgrade
...
1.  **Install Libraries:** Ensure `react-beautiful-dnd` (or similar) is installed for Ordering/Matching questions.
2.  **Update Types:** Update your TypeScript interfaces to reflect the new nested arrays (`ordering_items`, `matching_pairs`, `blanks`).
3.  **Component Switcher:** Create a `<QuestionRenderer />` component that takes a `question` prop and returns the correct sub-component (`<OrderingQuestion />`, `<MatchingQuestion />`, etc.).
4.  **Math Rendering:** Test the Lesson View with a Math/Physics lesson to ensure LaTeX renders correctly.
