...
class Command(BaseCommand):
    help = 'Create Scientific Activity lessons for CM1 (Cours Moyen 1)'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing CM1 Scientific Activity lessons before creating new ones',
        )

    def handle(self, *args, **options):
        lessons_data = [
            {'order': 1, 'title_arabic': 'تغيرات الحالة', 'title': 'State Changes'},
            {'order': 2, 'title_arabic': 'التغذية', 'title': 'Nutrition'},
            {'order': 3, 'title_arabic': 'الحرارة', 'title': 'Heat'},
            {'order': 4, 'title_arabic': 'الحركة', 'title': 'Movement'},
            {'order': 5, 'title_arabic': 'الغازات', 'title': 'Gases'},
            {'order': 6, 'title_arabic': 'الماء و الطبيعة', 'title': 'Water and Nature'},
            {'order': 7, 'title_arabic': 'الكهرباء', 'title': 'Electricity'},
            {'order': 8, 'title_arabic': 'تصنيف الحيوانات الفقرية', 'title': 'Classification of Vertebrate Animals'},
            {'order': 9, 'title_arabic': 'النباتات', 'title': 'Plants'},
            {'order': 10, 'title_arabic': 'دورة الحياة', 'title': 'The Life Cycle'},
        ]

        lessons_s1 = lessons_data[:5]
        lessons_s2 = lessons_data[5:]
