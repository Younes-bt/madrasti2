    ...
    const modules = [
        {
            ...
            description: t('parentHome.profileDesc', 'View your personal info'),
            icon: User,
            route: '/(parent-features)/profile',
        ...
        {
            id: 'kids',
            title: t('parentHome.myKids', 'My Kids'),
            description: t('parentHome.kidsDesc', 'Monitor your children'),
            icon: Users,
            route: '/(parent-features)/kids',
        ...
        {
            id: 'communication',
            title: t('parentHome.communication', 'Communication'),
            description: t('parentHome.communicationDesc', 'Messages & announcements'),
            icon: MessageSquare,
        ...
        {
            id: 'payments',
            title: t('parentHome.payments', 'Payments'),
            description: t('parentHome.paymentsDesc', 'Fees & invoices'),
            icon: CreditCard,
        ...
        {
            id: 'events',
            title: t('parentHome.events', 'Events'),
            description: t('parentHome.eventsDesc', 'School calendar & events'),
            icon: Calendar,
            route: '/(parent)/events',
            ...
            <SafeAreaView className="flex-1">
                ...
                <View className="px-6 pt-2 pb-6 flex-row justify-between items-center">
                    <View>
                        <Text className={`${subTextColor} text-xs font-medium uppercase tracking-wider mb-0.5`}>
                            {t('parentHome.parentLabel', 'Parent')}
                        </Text>
                        <Text className={`${textColor} text-2xl font-bold`}>
                            {t('parentHome.tabs.home', 'Home')}
                        </Text>
                    </View>
                ...
                >
                    ...
                    <Pressable
                        ...
                        onPress={() => router.push('/(parent-features)/profile')}
                    >
                        <View className="flex-row items-center gap-4">
                            <View className={`w-12 h-12 rounded-xl justify-center items-center overflow-hidden ${isDark ? 'bg-purple-500/20' : 'bg-purple-100'}`}>
                                {user?.avatar ? (
                                    <ImageBackground source={{ uri: user.avatar }} className="w-full h-full" resizeMode="cover" />
                                ) : (
                                    <Text className="text-purple-500 font-bold text-lg">{getUserInitials()}</Text>
                                )}
                            </View>
                            <View>
                                <Text className={`${textColor} text-base font-bold`}>{getUserName()}</Text>
                                <Text className={`${subTextColor} text-xs`}>{t('parentHome.parentLabel', 'Parent')}</Text>
                            </View>
                        </View>
                        <View className={`px-3 py-1.5 rounded-lg ${isDark ? 'bg-purple-500/10' : 'bg-purple-100'}`}>
                            <Text className={`${isDark ? 'text-purple-400' : 'text-purple-700'} text-xs font-semibold`}>{getUserInitials()}</Text>
                        </View>
                    </Pressable>
