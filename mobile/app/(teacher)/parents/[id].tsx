...
import { View, Text, ScrollView, SafeAreaView, Pressable, ActivityIndicator, Image, Linking } from 'react-native';
import { Stack, useLocalSearchParams, useRouter } from 'expo-router';
import { LinearGradient } from 'expo-linear-gradient';
import { useTranslation } from 'react-i18next';
// Adjusted import path for teacher directory
...
export default function ParentProfile() {
    ...
    const fetchParentDetails = React.useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} className="px-5 pt-2">
                    ...
                    <View className="mt-4 mb-6 items-center">
                        <View className="relative mb-4">
                            <View className={`h-28 w-28 rounded-3xl ${isDark ? 'bg-amber-500/20' : 'bg-amber-100'} items-center justify-center overflow-hidden border-2 ${isDark ? 'border-amber-400' : 'border-amber-200'} shadow-lg`}>
                    ...
                    <View className="flex-row gap-4 mb-8">
                        {parent.phone && (
                            <Pressable
                                ...
                                className="flex-1 bg-green-600 py-3 rounded-2xl flex-row items-center justify-center gap-2 active:opacity-90 shadow-lg shadow-green-900/20"
                            >
                        ...
                        {parent.email && (
                            <Pressable
                                ...
                                className="flex-1 bg-blue-600 py-3 rounded-2xl flex-row items-center justify-center gap-2 active:opacity-90 shadow-lg shadow-blue-900/20"
                            >
                            ...
                            {loadingChildren ? (
                                ...
                                <View className="gap-3">
                                    {children.map((child, index) => (
                                        <Pressable
                                            key={child.id || index}
                                            // Adjusted route for teacher navigation
                                            onPress={() => router.push({
