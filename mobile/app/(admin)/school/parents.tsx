...
export default function ParentsPage() {
    ...
    const fetchParents = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await api.get('/users/users/', { params: { role: 'PARENT' } });

            const parentsData = response.data?.results || (Array.isArray(response.data) ? response.data : []);
    ...
    }, [parents, statusFilter, searchQuery]);
    ...
    const getDisplayName = (parent: any) => {
        if (isRTL && (parent.ar_first_name || parent.ar_last_name)) {
            return `${parent.ar_first_name || ''} ${parent.ar_last_name || ''}`.trim();
        }
        return parent.full_name || `${parent.first_name || ''} ${parent.last_name || ''}`.trim();
    };

    const ParentCard = ({ parent }: { parent: any }) => (
        <Pressable
            onPress={() => router.push(`/(admin)/school/parents/${parent.id}` as any)}
            className={`rounded-3xl p-4 mb-4 ${cardBgColor} border ${borderColor} active:scale-[0.98] transition-transform`}
        >
            <View className="flex-row items-start gap-4">
                {/* Avatar */}
                <View className="relative">
                    <View className={`h-14 w-14 rounded-2xl ${isDark ? 'bg-amber-500/20' : 'bg-amber-100'} items-center justify-center overflow-hidden border ${isDark ? 'border-amber-400/30' : 'border-amber-200'}`}>
                        {parent.profile_picture_url ? (
                            <Image source={{ uri: parent.profile_picture_url }} className="h-full w-full" resizeMode="cover" />
                        ) : (
                            <Text className={`text-lg font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                                {(parent.first_name?.[0] || '') + (parent.last_name?.[0] || '')}
                            </Text>
                        )}
                    </View>
                    {/* Status Dot */}
                    <View className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 ${isDark ? 'border-[#1e293b]' : 'border-white'} ${parent.is_active ? 'bg-green-500' : 'bg-gray-400'}`} />
                </View>
            ...
            <SafeAreaView className="flex-1">
                ...
                <View className="px-6 pt-4 pb-2 flex-row items-center justify-between z-50">
                    ...
                    <Text className={`${textColor} text-xl font-bold tracking-wider`}>
                        {t('admin.school.modules.parents') || 'Parents'}
                    </Text>
                    {/* Add Parent Button */}
                    <Pressable
                        onPress={() => router.push('/(admin)/school/parents/add' as any)}
                        className={`w-10 h-10 items-center justify-center rounded-full ${isDark ? 'bg-amber-500/20 border-amber-400/30' : 'bg-amber-100 border-amber-200'} border active:opacity-70`}
                    >
                        <Plus size={24} color={isDark ? '#fbbf24' : '#d97706'} />
                    </Pressable>
                </View>
                ...
                <View className="px-6 py-4 space-y-4">
                    ...
                    <View className={`flex-row items-center px-4 h-12 rounded-2xl border ${borderColor} ${inputBg}`}>
                        ...
                        <TextInput
                            placeholder={t('common.search') || "Search parents..."}
                    ...
                    <View>
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-2">
                            ...
                            <Pressable
                                ...
                                className={`mx-1 px-4 py-2 rounded-full border ${statusFilter === 'all' ? 'bg-amber-600 border-amber-600' : `${cardBgColor} ${borderColor}`}`}
                            >
                            ...
                            <Pressable
                                ...
                                className={`mx-1 px-4 py-2 rounded-full border ${statusFilter === 'active' ? 'bg-amber-600 border-amber-600' : `${cardBgColor} ${borderColor}`}`}
                            >
                            ...
                            <Pressable
                                ...
                                className={`mx-1 px-4 py-2 rounded-full border ${statusFilter === 'inactive' ? 'bg-amber-600 border-amber-600' : `${cardBgColor} ${borderColor}`}`}
                            >
                ...
                {loading ? (
                    ...
                    <View className="flex-1 items-center justify-center px-6">
                        ...
                        <Pressable onPress={fetchParents} className="bg-amber-600 px-6 py-2 rounded-full">
                        ...
                        {filteredParents.length > 0 ? (
                            ...
                            <View className="py-20 items-center justify-center opacity-60">
                                ...
                                <Text className={`mt-4 text-center ${textColor}`}>
                                    {t('common.noResults') || "No parents found"}
                                </Text>
                            </View>
                        )}
                    </ScrollView>
