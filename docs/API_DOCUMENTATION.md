...
Comprehensive assignment system with auto-grading, gamification, and detailed analytics.
...
PATCH /api/homework/assignments/{id}/
...
```json
{
  ...
  "assignment_type": "enum",          // required: QCM|BOOK|OPEN|MIXED
  "subject": "integer",               // required, FK to Subject
  "subject_name": "string",           // read-only
  "grade": "integer",                 // required, FK to Grade  
  "grade_name": "string",             // read-only
...
GET /api/homework/assignments/?
  ...
  assignment_type=QCM&                 # QCM|BOOK|OPEN|MIXED
  subject=1&                           # subject ID
  grade=2&                            # grade ID
  classes=1,2,3&                      # comma-separated class IDs
  created_by=5&                       # teacher ID
  due_date__gte=2024-01-01&           # assignments due after date
  due_date__lte=2024-12-31&           # assignments due before date
  is_published=true&                  # published assignments only
...
POST /api/homework/assignments/{id}/unpublish/   # Unpublish assignment  
POST /api/homework/assignments/{id}/duplicate/   # Create duplicate
GET /api/homework/assignments/{id}/analytics/    # Detailed analytics
GET /api/homework/assignments/{id}/submissions/ # List all submissions
POST /api/homework/assignments/{id}/bulk_grade/ # Grade multiple submissions
```
...
{
  "id": "integer",
  "assignment": "integer",            // required, FK to Assignment
...
```json
{
  "id": "integer",
  "assignment": "integer",            // required, FK to Assignment
  "assignment_title": "string",       // read-only
  "student": "integer",               // required, FK to User (STUDENT)
  "student_name": "string",           // read-only
...
```json
{
  ...
  "reference_id": "integer",          // FK to related object (assignment, etc.)
...
**Transaction Reasons:**
- `ASSIGNMENT_COMPLETION` - Completed assignment
- `PERFECT_SCORE` - Got 100% on assignment
...
```json
{
  ...
  "details": {
    ...
    "assignment_type": ["Must be one of: QCM, BOOK, OPEN, MIXED."]
  }
}
```
...
```json
{
  ...
  "timestamp": "2024-01-15T10:30:00Z",
  "details": {
    "required_permissions": ["can_manage_attendance"],
    "user_role": "STUDENT",
...
```json
{
  ...
  "next": "http://localhost:8000/api/assignments/?page=3",
  "previous": "http://localhost:8000/api/assignments/?page=1", 
  "page_info": {
    "current_page": 2,
    "total_pages": 8,
    "page_size": 20,
