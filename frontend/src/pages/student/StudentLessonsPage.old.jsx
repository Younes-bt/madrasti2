...
const PAGE_SIZE = 50  // Increased from 12 to show more lessons per page
...
const StudentLessonsPage = () => {
  ...
  useEffect(() => {
    const loadSubjects = async () => {
      ...
      try {
        // Load subjects from actual lessons instead of grade-subjects relationship
        // This ensures we only show subjects that actually have lessons
        const response = await lessonsService.getLessons({
          grade: studentGrade.id,
          is_active: true,
          page_size: 1000, // Get all lessons to extract subjects
        })
        ...
        console.log(`Loaded ${uniqueSubjects.length} subjects from ${lessonList.length} lessons`)
      } catch (err) {
        console.warn('Failed to load subjects from lessons.', err)
        setSubjects([])
      }
    }
  ...
  useEffect(() => {
    const loadLessons = async () => {
      ...
      try {
        ...
        // Populate subjects from lessons if not already loaded
        if (subjects.length === 0 && lessonList.length > 0) {
          const subjectMap = new Map()
          lessonList.forEach((lesson) => {
            const subjectDetails = lesson.subject_details || {
              id: lesson.subject,
          ...
          if (subjectMap.size > 0) {
            const uniqueSubjects = Array.from(subjectMap.values())
            setSubjects(uniqueSubjects)
            console.log(`Fallback: Loaded ${uniqueSubjects.length} subjects from current page`)
          }
        }
      } catch (err) {
