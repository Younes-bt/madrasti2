...
const EditAcademicYearPage = () => {
  ...
  const fetchYearData = async () => {
    ...
    try {
      ...
      const yearData = response.data || response;
      
      setFormData({
        year: yearData.year || '',
        start_date: yearData.start_date || '',
        end_date: yearData.end_date || '',
        is_current: yearData.is_current || false
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await apiMethods.patch(`schools/academic-years/${yearId}/`, formData);
      toast.success(t('academicYears.updatedSuccessfully'));
      navigate('/admin/academic-management/academic-years');
    } catch (error) {
  ...
  if (initialLoading) {
    return (
      <AdminPageLayout
        title={t('academicYears.editAcademicYear')}
