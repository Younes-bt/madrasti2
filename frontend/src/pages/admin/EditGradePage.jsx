...
import { Hash, Save, ArrowLeft, AlertCircle, BookOpen, Target, GraduationCap, Star, Layers, Loader2 } from 'lucide-react';
...
const EditGradePage = () => {
  ...
  const fetchGrade = async () => {
    try {
      ...
      setFormData({
        educational_level: gradeData.educational_level?.toString() || '',
        grade_number: gradeData.grade_number?.toString() || '',
        name: gradeData.name || '',
        name_arabic: gradeData.name_arabic || '',
        name_french: gradeData.name_french || '',
        passing_grade: gradeData.passing_grade?.toString() || '10.00'
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiMethods.patch(`schools/grades/${id}/`, submitData);
  ...
  if (initialLoading) {
    return (
      <AdminPageLayout
        title={t('grades.editGrade')}
        subtitle={t('grades.editGradeSubtitle')}
