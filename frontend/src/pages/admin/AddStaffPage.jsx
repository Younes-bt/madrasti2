...
const AddStaffPage = () => {
    ...
    try {
      ...
      if (error.response?.status >= 400 && error.response?.status < 500) {
        if (error.response?.data) {
          ...
          toast.error(errorData.error || errorData.detail || t('error.createStaffFailed'));
        } else {
          toast.error(t('error.createStaffFailed'));
        }
      } else {
        // For other errors (network issues, 5xx errors, etc.), assume creation might have succeeded
        console.warn('Non-client error occurred, staff member might have been created successfully');
        toast.info('Request completed but with some issues. Please check if the staff member was created.');
  ...
  return (
    <AdminPageLayout
      title={t('staff.addNewStaff')}
      subtitle={t('staff.addNewStaffDescription')}
      showBackButton={true}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    disabled
                    placeholder={t('staff.emailWillBeGenerated')}
                  />
                </div>
                <p className="text-xs text-gray-500">{t('staff.emailGeneratedInfo')}</p>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {t('staff.accountInformation')}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <h4 className="font-medium text-blue-900 mb-2">{t('staff.accountDetails')}</h4>
                <div className="space-y-2 text-sm text-blue-800">
                  <div className="flex items-center justify-between">
                    <span>{t('common.role')}:</span>
                    <span className="font-medium">{t('roles.staff')}</span>
                  </div>
                  <div className="flex items-center justify-between">
